...
const GRAMMAR_PATH = process.argv[2] || path.join(__dirname, '..', 'grammar_complete.md');
...
async function loadAndChunkMarkdown(filePath) {
  try {
    ...
    const chunks = [];
    let currentChunk = { heading: '', content: '', level: 0, headingNode: null };
    let inChunk = false;
    ...
    function visit(node, parent) {
      if (node.type === 'heading' && node.depth <= 3) {
        ...
        inChunk = true;
      } else if (inChunk && (node.type === 'paragraph' || node.type === 'code' || node.type === 'table')) {
...
async function callLLM(systemPrompt, userContent, maxTokens = 2048, examples = []) {
  const MAX_RETRIES = 2;
  let retries = 0;
  ...
  while (true) {
    try {
      ...
      if (retries < MAX_RETRIES) {
        retries++;
        console.warn(`API error, retrying (${retries}/${MAX_RETRIES}): ${error.message}`);
...
`;
  ...
  try {
    ...
    if (!result.includes('ID,Parameter_ID,Language_ID,Value,Source')) {
      console.warn(`Invalid CLDF output for ${chunk.heading}. Using header only.`);
    ...
    try {
      ...
      console.warn(`CSV validation failed for ${chunk.heading}:`, parseError.message);
    }
    
    return result;
  } catch (error) {
    console.error(`Error extracting CLDF from ${chunk.heading}:`, error.message);
    return 'ID,Parameter_ID,Language_ID,Value,Source\n';
  }
}
  ...
  const examples = [
    {
      ...
      assistant: '{"items":[{"transcript":"Nyulu jalbu-ngku karrkay kawa-ny","gloss":[{"morpheme":"nyulu","gloss":"3SG.NOM"},{"morpheme":"jalbu-ngku","gloss":"woman-ERG"},{"morpheme":"karrkay","gloss":"child.ABS"},{"morpheme":"kawa-ny","gloss":"look.after-PAST"}],"translation":"The woman looked after the child.","source":"Example (12)"}]}'
    }
  ];
      ...
      if (Array.isArray(parsed.items)) {
        ...
        try {
          ...
          const xigtSchema = JSON.parse(
            await fs.readFile(path.join(__dirname, 'schemas/xigt.json'), 'utf8')
          );
          const validateIGT = ajv.compile(xigtSchema);
          
          if (!validateIGT({ items: parsed.items })) {
            console.warn(`IGT validation failed for ${chunk.heading}:`, validateIGT.errors);
          }
        } catch (schemaError) {
          console.warn(`Schema validation error for ${chunk.heading}:`, schemaError.message);
        }
        
        return parsed.items;
      }
    } catch (parseError) {
      console.warn(`Failed to parse IGT JSON from ${chunk.heading}:`, parseError.message);
    }
...
`;
  ...
  try {
    ...
    try {
      // Extract JSON from the response if it's not pure JSON
      const jsonMatch = result.match(/\{\s*"@context".*?"@graph"\s*:\s*\[.*?\]\s*\}/s);
      const jsonStr = jsonMatch ? jsonMatch[0] : result;
      ...
      console.warn(`Failed to parse OntoLex JSON from ${chunk.heading}:`, parseError.message);
    }
    
    return [];
  } catch (error) {
    console.error(`Error extracting OntoLex from ${chunk.heading}:`, error.message);
    return [];
  }
}
...
async function saveResults(cldfRows, igtItems, ontolexEntries) {
  try {
    ...
    await fs.writeFile(path.join(OUTPUT_DIR, 'grammar_features.csv'), cldfContent);
    ...
    console.log(`Saved ${ontolexEntries.length} OntoLex entries to lexicon.jsonld`);
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await fs.writeFile(xigtSchemaPath, JSON.stringify(basicSchema, null, 2));
      }
    }
    ...
    for (let i = 0; i < chunks.length; i++) {
      const chunk = chunks[i];
      console.log(`Processing chunk ${i+1}/${chunks.length}: ${chunk.heading}`);
    ...
    console.log(`Extracted ${cldfRows.length - 1} CLDF features, ${igtItems.length} IGT examples, and ${ontolexEntries.length} OntoLex entries.`);
  } catch (error) {
