...
RUN_ID="${RUN_ID:-v21.1-codex-synthetic-direct-gvn-3epoch-lr2e-5}"
GATE_ID="${GATE_ID:-${RUN_ID}-gate128-e100}"
PYTHON="${PYTHON:-/opt/mobtranslate-mt-venv/bin/python}"
DATA_ROOT="${DATA_ROOT:-/workspace/v21.1-codex/data}"
BASE_MODEL="${BASE_MODEL:-/workspace/v21.1-codex/base_model/merged}"
OUT_ROOT="${OUT_ROOT:-/workspace/v21.1-codex/models}"
LOG_ROOT="${LOG_ROOT:-/workspace/v21.1-codex/logs}"
REPORT_ROOT="${REPORT_ROOT:-/workspace/v21.1-codex/reports}"
...
cd "$DATA_ROOT"
sha256sum -c SHA256SUMS.v21.1 | tee "$LOG_ROOT/$RUN_ID.data-checksums.log"
...
print(json.dumps({
    "run_id": sys.argv[1],
    "owner": "codex",
    "lane": "v21.1-codex-synthetic-direct",
...
START_UTC="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
{
  ...
  echo
  echo "Treatment: 16,642 source-disjoint synthetic rows only; no Bible, DB-usage, or elder replay."
  echo
...
"$PYTHON" "$SCRIPT_DIR/train_nllb_lora.py" \
  ...
  --lora-r "$LORA_R" --lora-alpha "$LORA_ALPHA" --lora-dropout "$LORA_DROPOUT" \
  --lora-target-modules "$LORA_TARGET_MODULES" \
  --eval-steps 1040 --save-steps 1040 --save-total-limit 2 --logging-steps 100 \
  --generation-num-beams 1 --generation-no-repeat-ngram-size 0 \
  --generation-repetition-penalty 1.0 --generation-length-penalty 1.0 \
  --seed "$SEED" --no-shuffle-before-cap \
  2>&1 | tee -a "$LOG_FILE"
