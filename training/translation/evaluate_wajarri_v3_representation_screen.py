#!/usr/bin/env python3
"""Evaluate all Wajarri terminology representations in one model load."""
...
def score_row(
    ...
    return {
        ...
        "evaluation_condition": row["evaluation_condition"],
...
def summarize_by_condition(rows: list[dict[str, Any]]) -> dict[str, Any]:
    conditions = sorted({str(row["evaluation_condition"]) for row in rows})
    return {
        condition: summarize(
            [row for row in rows if row["evaluation_condition"] == condition]
        )
...
def main() -> None:
    ...
    summary = {
        ...
        "metrics_by_condition": summarize_by_condition(
            predictions_by_batch[primary_size]
        ),
        "metrics_by_batch_size": {
            str(size): summarize(predictions)
            for size, predictions in predictions_by_batch.items()
        },
        "metrics_by_condition_and_batch_size": {
            str(size): summarize_by_condition(predictions)
            for size, predictions in predictions_by_batch.items()
        },
        ...
        "claim_limit": "Paired controlled-synthetic representation evidence; not natural-language reliability, speaker validation, or release authorization.",
    }
