...
RUN_ID=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["run_id"])' "$KIT_DIR/CONTRACT.json")
OUTPUT_DIR=${1:-/workspace/${RUN_ID}-output}
VENV=${WBV_VENV:-/workspace/wbv-v3-forced-slot-confirmation-venv}
...
if contract["claims"]["sealed_test_included"] is not False:
    raise SystemExit("Confirmation kit must not include the sealed Wajarri test")
if contract["experiment_kind"] != "forced_subject_slot_confirmation":
    raise SystemExit("Unexpected experiment kind")
PY
