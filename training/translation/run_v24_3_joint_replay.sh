...
WORK_ROOT="${WORK_ROOT:-/workspace/v24.3-joint-lexeme-dose29-rerun1}"
...
INPUT_SHA256_FILE="${INPUT_SHA256_FILE:-$WORK_ROOT/SHA256SUMS.v24.3-rerun1-inputs}"
...
DRIVER_LOG="$LOG_ROOT/v24.3-driver.log"
RESOURCE_LOG="$LOG_ROOT/v24.3-resource.csv"
...
required_paths=(
  ...
  "$SCRIPT_DIR/summarize_v24_lexeme_trajectories.py"
)
...
[[ "$(wc -l < "$TRAIN_FILE")" -eq 115136 ]] || { echo "Training row count is not 115,136" >&2; exit 4; }
...
jq -e '
  ...
  and .mixture.lexeme_repetitions == 29
  and .mixture.lexeme_presentations == 78996
  and .mixture.sentence_unique_rows == 18070
  and .mixture.sentence_repetitions == 2
  and .mixture.sentence_presentations == 36140
  and .mixture.train_rows == 115136
...
contract = {
    "schema_version": 1,
    "run_id": "v24.3-joint-lexeme-dose29-rerun1",
    "started_at": started_at,
    "causal_question": "additive lexical-dose response with sentence replay held constant",
    "hypothesis": (
        "Increasing every governed lexeme from 13 to 29 explicit presentations, while retaining the exact "
        "v24.2 non-Bible sentence set at two presentations, can reach the frozen closed-set lexical gate "
        "without the sentence compression observed in v24.1."
    ),
    "base_model": "exact v21.2 merged weights and tokenizer",
    "comparison_run": "v24.2 J50: 13 lexical and 2 sentence presentations",
    "arm": "J69",
    "seed": 17,
    "learning_rate": 2e-4,
    "planned_max_steps": 3598,
    "checkpoint_steps": [900, 1800, 2700, 3598],
    "micro_batch_size": 32,
    "gradient_accumulation_steps": 1,
    "train_rows": 115136,
    "planned_presentations_per_generated_row": 1,
    "row_mixture": {"lexeme": 78996, "sentence": 36140, "bible": 0},
    "task_token": {"token": "<lexeme>", "single_special_token": True, "trainable_embedding_row": True},
    "controlled_constants": [
        "base weights", "tokenizer", "sentence identities", "sentence repetitions", "seed",
        "learning rate", "LoRA topology", "batch size", "length caps", "decoder", "evaluation data",
    ],
    "changed_factors": ["lexeme repetitions", "total updates", "learning-rate trajectory length"],
    "evaluation": {
        "all_lexeme_census_rows": 2724,
        "all_lexeme_census_at_every_checkpoint": True,
        "one_prediction_per_lexeme_per_checkpoint": True,
        "accepted_reference_scoring": True,
        "exhaustive_failure_ledger": True,
        "checkpoint_trajectory_analysis": True,
        "retention_rows": 2086,
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] train $ARM lr=$LEARNING_RATE steps=$MAX_STEPS" | tee -a "$DRIVER_LOG"
"$PYTHON" "$SCRIPT_DIR/train_nllb_lora.py" \
  --base-model "$BASE_MODEL" --train-file "$TRAIN_FILE" --validation-file "$MONITOR_FILE" \
  --output-dir "$MODEL_ROOT/$ARM" --model-version "v24.3-$ARM-rerun1" --run-id "v24.3-$ARM-seed$SEED-rerun1" \
...
jq -e \
  ...
  '.trainer_state.global_step == $steps
   ...
   and .dataset.split_rows.train == 115136
   and .dataset.split_rows.validation == 128
   and .trainer_state.actual_training_exposure.examples == 115136
   and .trainer_state.actual_training_exposure.unique_rows_seen == 115136
   ...
   and .trainer_state.actual_training_exposure.by_task.dictionary_lexeme.examples == 78996
...
expected_saved_steps=(900 1800 2700 3598)
[[ "${#checkpoints[@]}" -eq "${#expected_saved_steps[@]}" ]] || {
  echo "Expected four saved checkpoints; found ${#checkpoints[@]}" >&2
  exit 5
}
[[ -d "$MODEL_ROOT/$ARM/adapter" ]] || { echo "Final adapter is absent" >&2; exit 5; }
final_adapter_sha="$(sha256sum "$MODEL_ROOT/$ARM/adapter/adapter_model.safetensors" | awk '{print $1}')"
final_checkpoint_sha="$(sha256sum "$MODEL_ROOT/$ARM/checkpoint-$MAX_STEPS/adapter_model.safetensors" | awk '{print $1}')"
[[ "$final_adapter_sha" == "$final_checkpoint_sha" ]] || {
  echo "Final adapter differs from checkpoint $MAX_STEPS" >&2
  exit 5
}
...
run_eval() {
  ...
  local args=(
    ...
    --dtype float32 --require-cuda --deterministic --seed 0 --adapter-dir "$adapter_dir"
  )
...
evaluate_candidate() {
  local label="$1" adapter_dir="$2"
  local curated_output="$PREDICTION_ROOT/$label.curated.json"
  local retention_output="$PREDICTION_ROOT/$label.retention.json"
  run_eval "$label-curated" "$CURATED_EVAL" "$curated_output" "$adapter_dir"
  run_eval "$label-retention" "$DEVELOPMENT_SUITE" "$retention_output" "$adapter_dir"
...
for index in "${!checkpoints[@]}"; do
  ...
  step="${checkpoint##*-}"
  [[ "$step" -eq "${expected_saved_steps[$index]}" ]] || {
    echo "Unexpected checkpoint sequence at index $index: $step" >&2
    exit 5
  }
  evaluate_candidate "${ARM}_s$(printf '%04d' "$step")" "$checkpoint"
done
...
"$PYTHON" "$SCRIPT_DIR/select_v24_screen.py" \
  ...
  --bootstrap-samples 10000 --seed v24.3-joint-lexeme-dose29-retention \
...
"$PYTHON" "$SCRIPT_DIR/analyze_v24_lexicon_errors.py" \
  ...
  --seed v24.3-joint-lexeme-dose29-all-lexeme-census \
...
print(json.dumps({"failure_rows_by_candidate": counts, "output": str(output)}, sort_keys=True))
PY

trajectory_args=()
for label in "${candidate_order[@]}"; do
  trajectory_args+=(--candidate-order "$label")
done
"$PYTHON" "$SCRIPT_DIR/summarize_v24_lexeme_trajectories.py" \
  --row-analysis-file "$ANALYSIS_ROOT/all-checkpoint-row-analysis.jsonl" \
  "${trajectory_args[@]}" --baseline-label B0 \
  --output "$ANALYSIS_ROOT/all-checkpoint-trajectory-summary.json" \
  --matrix-output "$ANALYSIS_ROOT/all-checkpoint-trajectory-matrix.jsonl" \
  | tee "$LOG_ROOT/all-checkpoint-trajectory.stdout.json"
...
"$PYTHON" - \
  ...
  "$ANALYSIS_ROOT/all-checkpoint-trajectory-summary.json" \
...
lexical_path, retention_path, qualitative_path, trajectory_path, manifest_path, output_path = sys.argv[1:]
...
result = {
    ...
    "checkpoint_assessments": sorted(records, key=lambda record: record["step"]),
...
echo "[$END_UTC] v24.3 joint lexical-dose run complete; finalizing checksum ledger" | tee -a "$DRIVER_LOG"
