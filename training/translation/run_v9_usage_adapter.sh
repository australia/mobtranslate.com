...
RUN_ID="${RUN_ID:-v9.9A-usage-only-adapter-tpi-80epoch-batch16}"
...
EVAL_STEPS="${EVAL_STEPS:-460}"
SAVE_STEPS="${SAVE_STEPS:-460}"
LOGGING_STEPS="${LOGGING_STEPS:-92}"
SEED="${SEED:-42}"
...

{
  ...
  echo
  echo "Purpose: usage-example-only adapter diagnostic."
  echo
  echo "Training file: $TRAIN_USAGE_FILE"
  echo "Validation file: $TRAIN_USAGE_FILE"
...
"$PYTHON" train_nllb_lora.py \
  ...
  --validation-file "$TRAIN_USAGE_FILE" \
  --test-file "$TRAIN_USAGE_FILE" \
  --output-dir "$OUT_DIR" \
  --direction "$DIRECTION" \
