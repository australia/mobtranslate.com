#!/usr/bin/env python3
"""Build an immutable Wajarri copy/composition RunPod screening kit."""
...
METHOD_ID = "wajarri-v3-copy-composition-runpod-kit-v1"
SCREEN_INPUTS = {
    ...
    "copy_screen": ("copy_development_screen", "copy"),
}
...
def build_screen_rows(inputs: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    ...
    endpoint_counts = {
        endpoint: sum(row["evaluation_endpoint"] == endpoint for row in combined)
        for endpoint in SCREEN_INPUTS
    }
    if any(count <= 0 for count in endpoint_counts.values()):
        raise ValueError("combined screen lacks an endpoint")
    return sorted(combined, key=lambda row: (row["evaluation_endpoint"], row["id"]))


def build_confirmation_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return sorted(
        [
            evaluation_row(row, endpoint="copy_confirmation", condition="copy")
            for row in rows
        ],
...
def main() -> None:
    ...
    if build.get("schema_version") != 1 or build.get("method_id") != METHOD_ID:
        raise ValueError("unexpected copy/composition kit contract")
    ...
    input_rows = {
        ...
        for name, path in bound_inputs.items()
        if name in {input_name for input_name, _ in SCREEN_INPUTS.values()}
        or name == "copy_development_confirmation"
    }
    repo_sources = {
    ...
    confirmation_rows = build_confirmation_rows(
        input_rows["copy_development_confirmation"]
    )
    confirmation_path = output_dir / "payload/data/COPY-CONFIRMATION.jsonl"
    ...
    final_contract = {
        ...
        "copy_confirmation": {
            "path": "payload/data/COPY-CONFIRMATION.jsonl",
    ...
    report = {
        "status": "PASS_AUTHORIZED_COPY_COMPOSITION_KIT_BUILT_GPU_EXECUTION_ALLOWED",
        "run_id": build["run_id"],
        "files": len(files),
        ...
        "copy_confirmation_rows": len(confirmation_rows),
