#!/usr/bin/env python3
"""Run the runtime-bound local-only Wajarri Stage-I full lexical census."""
...
def absolute_without_resolving(path: Path) -> Path:
    """Return an absolute path while preserving a virtual-environment symlink."""
...
def immutable_directory_identity(path: Path) -> dict[str, Any]:
    if not path.is_dir():
    ...
    if symlinks:
        raise ContractError(f"runtime overlay contains symlinks: {symlinks}")
...
def verify_contract(
    ...
    expected_python = absolute_without_resolving(
        resolve(program_root, runtime["python_executable_path"])
    )
    if invoked_python != expected_python:
        raise ContractError(
            "invoked Python path drift: "
            f"expected {expected_python}, observed {invoked_python}"
        )
    running_python = absolute_without_resolving(Path(sys.executable))
    if running_python != expected_python:
        raise ContractError(
            "running Python path drift: "
            f"expected {expected_python}, observed {running_python}"
        )
    if not expected_python.is_file() or not os.access(expected_python, os.X_OK):
        raise ContractError(f"bound Python is not executable: {expected_python}")
    resolved_python = expected_python.resolve()
    if str(resolved_python) != runtime["python_resolved_path"]:
        raise ContractError("bound Python symlink target drift")
    require_sha(
        resolved_python,
        runtime["python_target_sha256"],
        "bound Python target",
    )
    runtime_manifest = resolve(program_root, runtime["manifest_path"])
    require_sha(
        runtime_manifest,
    ...
    overlay_identity = immutable_directory_identity(overlay)
    expected_overlay_identity = runtime["overlay_identity"]
    for key in ("algorithm", "aggregate_sha256", "file_count", "total_bytes"):
        if overlay_identity[key] != expected_overlay_identity[key]:
            raise ContractError(
                f"runtime overlay {key} drift: expected "
                f"{expected_overlay_identity[key]}, observed {overlay_identity[key]}"
            )
    overlay_requirements = resolve(program_root, runtime["overlay_requirements_path"])
    require_sha(
        overlay_requirements,
        runtime["overlay_requirements_sha256"],
        "runtime overlay requirements",
    )
    ...
    return {
        ...
        "runtime_overlay_identity": {
            key: overlay_identity[key]
            for key in ("algorithm", "aggregate_sha256", "file_count", "total_bytes")
        },
        "runtime_overlay_requirements": {
            "path": str(overlay_requirements),
            "sha256": runtime["overlay_requirements_sha256"],
        },
        "runtime_manifest": {
        ...
        "python_binding": {
            ...
            "target_sha256": runtime["python_target_sha256"],
        },
    }
...
def main() -> None:
    ...
    preflight = verify_contract(contract, program_root, workspace_root, python)
    preflight.update(
        {
