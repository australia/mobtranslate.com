...
RUN_ID="${RUN_ID:-v20.0-full-candidate-corpus-gvn-3epoch-lr2e-5}"
PYTHON="${PYTHON:-/opt/mobtranslate-mt-venv/bin/python}"
DATA_ROOT="${DATA_ROOT:-/workspace/data/kuku_yalanji_v20_0_full_candidate_corpus_gvn}"
...
MAX_SOURCE_LENGTH="${MAX_SOURCE_LENGTH:-320}"
...
EVAL_STEPS="${EVAL_STEPS:-1100}"
SAVE_STEPS="${SAVE_STEPS:-1100}"
LOGGING_STEPS="${LOGGING_STEPS:-220}"
MAX_VALIDATION_SAMPLES="${MAX_VALIDATION_SAMPLES:-1024}"
MAX_TEST_SAMPLES="${MAX_TEST_SAMPLES:-1024}"
SEED="${SEED:-42}"

TRAIN_FILE="${TRAIN_FILE:-$DATA_ROOT/train.eng-gvn.jsonl}"
VALIDATION_FILE="${VALIDATION_FILE:-$DATA_ROOT/validation.eng-gvn.jsonl}"
TEST_FILE="${TEST_FILE:-$DATA_ROOT/test.eng-gvn.jsonl}"
EVAL_TRAIN_SAMPLE_FILE="${EVAL_TRAIN_SAMPLE_FILE:-$DATA_ROOT/bible/eval_train_sample.eng-gvn.jsonl}"
VALIDATION_BIBLE_DIRECT_FILE="${VALIDATION_BIBLE_DIRECT_FILE:-$DATA_ROOT/bible/validation_direct.eng-gvn.jsonl}"
VALIDATION_BIBLE_REF_FILE="${VALIDATION_BIBLE_REF_FILE:-$DATA_ROOT/bible/validation_ref.eng-gvn.jsonl}"
TEST_BIBLE_DIRECT_FILE="${TEST_BIBLE_DIRECT_FILE:-$DATA_ROOT/bible/test_direct.eng-gvn.jsonl}"
TEST_BIBLE_REF_FILE="${TEST_BIBLE_REF_FILE:-$DATA_ROOT/bible/test_ref.eng-gvn.jsonl}"
...
files = [
    ...
    "test.eng-gvn.jsonl",
    "bible/eval_train_sample.eng-gvn.jsonl",
    "bible/validation_direct.eng-gvn.jsonl",
    "bible/validation_ref.eng-gvn.jsonl",
    "bible/test_direct.eng-gvn.jsonl",
    "bible/test_ref.eng-gvn.jsonl",
...
for rel in files:
    ...
    p99 = sorted_lens[int(len(sorted_lens) * 0.99) - 1] if sorted_lens else 0
    results.append(
        {
            "file": rel,
            "rows": len(lens),
            ...
            "first_over_max_source_length": over[:12],
        }
    )
...

{
  ...
  echo
  echo "Purpose: full 20,911-pair candidate-corpus continuation from v12 with tagged direct/reference Bible tasks, DB usage replay, and elder-shared sentence-pair replay."
  echo
  ...
  echo "Training eval cap: validation=$MAX_VALIDATION_SAMPLES, test=$MAX_TEST_SAMPLES."
  echo
  echo "## Dataset Manifest"
  echo
  echo '```json'
  cat "$DATA_ROOT/v20_full_candidate_corpus_manifest.json"
  echo '```'
  echo
...
evaluate_split "train_sample" "$EVAL_TRAIN_SAMPLE_FILE"
evaluate_split "validation_bible_direct" "$VALIDATION_BIBLE_DIRECT_FILE"
evaluate_split "validation_bible_ref" "$VALIDATION_BIBLE_REF_FILE"
evaluate_split "test_bible_direct" "$TEST_BIBLE_DIRECT_FILE"
evaluate_split "test_bible_ref" "$TEST_BIBLE_REF_FILE"
evaluate_split "heldout_usage" "$HELDOUT_USAGE_FILE"
evaluate_split "elder_sentence_pair_all" "$ELDER_SENTENCE_PAIR_FILE"
evaluate_split "test_all" "$TEST_FILE"
...
def analyze(filename):
    ...
    for row in rows:
        ...
        task_counts[(row.get("task_tagging") or {}).get("task") or row.get("pair_kind") or "unknown"] += 1
        tier_counts[row.get("tier") or "none"] += 1
    ...
    return {
        ...
        "elder_sentence_pair_exact": {
            "rows": elder_rows,
            "exact": elder_exact,
            "rate": elder_exact / elder_rows if elder_rows else None,
        },
    }

analysis = {
    "train_sample": analyze("eval_train_sample_predictions_greedy.json"),
    "validation_bible_direct": analyze("eval_validation_bible_direct_predictions_greedy.json"),
    "validation_bible_ref": analyze("eval_validation_bible_ref_predictions_greedy.json"),
    "test_bible_direct": analyze("eval_test_bible_direct_predictions_greedy.json"),
    "test_bible_ref": analyze("eval_test_bible_ref_predictions_greedy.json"),
    ...
    "test_all": analyze("eval_test_all_predictions_greedy.json"),
}
