...
RUN_ID="${RUN_ID:-v18.0-usage-elder-sentence-continuation-from-v10-tpi-10epoch-lr5e-5}"
PYTHON="${PYTHON:-/opt/mobtranslate-mt-venv/bin/python}"
DATA_ROOT="${DATA_ROOT:-/workspace/data/kuku_yalanji_v18_0_v10_plus_elder_sentence_pair}"
...
EVAL_STEPS="${EVAL_STEPS:-620}"
SAVE_STEPS="${SAVE_STEPS:-620}"
LOGGING_STEPS="${LOGGING_STEPS:-124}"
SEED="${SEED:-42}"

TRAIN_FILE="${TRAIN_FILE:-$DATA_ROOT/db_usage_plus_elder_sentence_pair/train_usage_elder_sentence_pair_oversampled.eng-gvn.jsonl}"
EVAL_TRAIN_FILE="${EVAL_TRAIN_FILE:-$DATA_ROOT/db_usage_plus_elder_sentence_pair/eval_train_usage_elder_sentence_pair.eng-gvn.jsonl}"
...
files = [
    ...
    "db_usage_plus_elder_sentence_pair/eval_train_usage_elder_sentence_pair.eng-gvn.jsonl",
...

{
  ...
  echo
  echo "Purpose: domain-separated usage plus elder-shared sentence-pair continuation from v10.0 merged model."
  echo
  echo "Base model: $BASE_MODEL"
  echo "Training file: $TRAIN_FILE"
  echo "Eval train file: $EVAL_TRAIN_FILE"
  echo "Heldout usage file: $HELDOUT_USAGE_FILE"
  echo "Elder sentence-pair eval file: $ELDER_SENTENCE_PAIR_FILE"
  ...
  echo
  echo '```json'
  cat "$DATA_ROOT/v18_elder_sentence_pair_manifest.json"
  echo '```'
  echo
...

{
  ...
  echo "ELDER_SENTENCE_PAIR_FILE=$ELDER_SENTENCE_PAIR_FILE"
...
evaluate_split "train_usage_elder_sentence_pair" "$EVAL_TRAIN_FILE"
...
analysis = {
    "train_usage_elder_sentence_pair": analyze("eval_train_usage_elder_sentence_pair_predictions_greedy.json"),
...
echo "V18_USAGE_ELDER_SENTENCE_PAIR_CONTINUATION_DONE"
