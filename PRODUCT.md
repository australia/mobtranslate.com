...
The first run makes that promise legible before asking for an account: find words, prefer recorded human voices, and follow the source. A learner chooses a starting language and sees that collection’s source, publication, and stewardship status without any claim of endorsement. Search, source-backed daily practice, available recordings, and place-name exploration remain open without signing in; accounts are for contribution and permission controls. The home daily word must resolve to an addressable entry in the selected dictionary. If it cannot, the product shows the gap—it never fills Mi’gmaq, Wajarri, or Anindilyakwa with a Kuku Yalanji sample (or vice versa).
...
Voice contribution follows a stricter promise: **permission stays attached to purpose**. An ordinary recorder may allow Mob Translate to keep a clip and play it beside a public dictionary word or sentence, but that choice grants no AI or speech-model training, provider transfer, voice replication, model creation or distribution, public model metrics, commercial reuse, or public-domain dedication. The contributor confirms both that it is their voice and that they have authority to share the language content. They can withdraw public playback in the app; the storage and listing paths evaluate the current append-only consent event rather than treating an old checkbox as permanent authority. Broader speech research requires a separate, purpose-specific and appropriately governed process.

The curator-led Elder studio is that separate process, not a shortcut around it. A speaker-present permission conversation records retention, public listening, transcripts, speech recognition, computer speech, recognisable voice replication, hosted-provider transfer, model creation, model distribution, public metrics, and commercial use as distinct choices, all off by default. Each recording session reconfirms the speaker is present under the current immutable ledger version. Every machine-generated sentence remains visibly a draft until the Elder explicitly confirms or corrects its wording; opening the microphone never silently counts as language review. Native capture produces a measured mono 16-bit PCM WAV bound to the session and consent record. A waiting offline master keeps that same binding, can be deleted from the device, and cannot upload after the permission is replaced or withdrawn.

Place is presented as context, not a territorial claim. The Country surface names the language-specific region, links to an independent external language or Country reference without implying endorsement, and says plainly that the map is general orientation—not a fixed language boundary or land-claim record. Every place name can open its dictionary entry and source trail. The automated place-name walk plays only available human recordings; it never turns an unlabelled synthetic approximation into the voice of Country.
...
2. **Custodial confidence.** Show source, contributor, and approval state for every entry. Never route research model output into learner tools without explicit authorization. Research limitations stay visible and honest, not buried in tooltips.
   ...
   A voice is not just another file. Before the ordinary microphone opens, the interface states what public sharing permits, what it explicitly does not permit, whose voice is being recorded, what authority may be needed, how the public identity is labelled, and how to withdraw. A broader voice-project permission must never be silently reused by the dictionary recorder, and technical corpus coverage must never be presented as consent or project approval.
