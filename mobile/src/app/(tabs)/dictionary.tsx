...
import { LangPicker, Screen, SpeakerButton, Sub, Title } from '../../components/kit';
...
export default function DictionaryScreen() {
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(async () => {
      try { setHits(await searchWords(code, q.trim())); } catch { setHits([]); }
  ...
  return (
    <Screen scroll={false}>
      <Title>Words</Title>
      <Sub>Search the dictionary.</Sub>
      <LangPicker languages={languages} value={code} onChange={(c) => { setCode(c); }} />
      ...
      <FlatList
        data={hits}
        keyExtractor={(h) => h.wordId}
        keyboardShouldPersistTaps="handled"
        contentContainerStyle={{ paddingBottom: 24, gap: 10 }}
        ListEmptyComponent={
          !loading && q.trim().length >= 2 ? (
            <Text style={styles.empty}>No words found for “{q}”.</Text>
          ) : null
        }
        renderItem={({ item }) => (
          <Pressable
...
const styles = StyleSheet.create({
  searchRow: {
    ...
    borderRadius: radius.md, borderWidth: 1, borderColor: C.border, paddingHorizontal: 14, height: 56,
  },
  search: { flex: 1, fontSize: S.body, color: C.ink },
  row: {
    flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: C.surface,
    borderRadius: radius.md, borderWidth: 1, borderColor: C.border, padding: 14,
  },
  word: { fontSize: S.heading, fontWeight: '700', color: C.ink },
  meaning: { fontSize: S.label, color: C.muted, marginTop: 2 },
  empty: { fontSize: S.label, color: C.muted, textAlign: 'center', paddingVertical: 30 },
});
