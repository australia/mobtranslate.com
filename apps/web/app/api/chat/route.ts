...
async function primaryDefinitions(
  wordIds: string[],
  ...
  try {
    ...
    if (Number.isFinite(contentLength) && contentLength > MAX_CHAT_BODY_BYTES) {
      return Response.json(
        { error: 'That conversation is too large to send.' },
    ...
    const messageTexts = messages.map(uiMessageText);
    if (
      messageTexts.some((text) => text.length > MAX_CHAT_MESSAGE_CHARS) ||
      messageTexts.reduce((total, text) => total + text.length, 0) >
        MAX_CHAT_TEXT_CHARS
    ) {
      return Response.json(
    ...
    const profile = await safe(
      async () => null as { username?: string; display_name?: string } | null,
    );
    ...
    const sessions = await safe(async () => {
      const rows = await db
        ...
        .where(
          and(
            eq(quizSessionsT.userId, user.id),
            isNotNull(quizSessionsT.completedAt),
          ),
        );
      return rows.map((r) => ({
        ...
        languages: r.lang_id
          ? { id: r.lang_id, name: r.lang_name, code: r.lang_code }
          : null,
      }));
    });
    ...
    const wordStates = await safe(async () => {
      ...
      return rows.map((r) => ({
        ...
        words: {
          word: r.word,
          languages: { name: r.lang_name, code: r.lang_code },
        },
      }));
    });
      ...
      languages: Array.from(languageStats.values()).map((lang) => ({
        ...
        accuracy:
          lang.totalQuestions > 0
            ? Math.round((lang.correctAnswers / lang.totalQuestions) * 100)
            : 0,
    ...
    const lastUserText = uiMessageText(
...
- Liked Words: ${userContext.likedWords
        .slice(0, 5)
        .map((w) => `"${w.word}" (${w.language})`)
        .join(
          ', ',
        )}${userContext.likedWords.length > 5 ? ` and ${userContext.likedWords.length - 5} more` : ''}
- Mastered Words: ${userContext.masteredWords
        .slice(0, 5)
        .map((w) => `"${w.word}" (${w.language})`)
        .join(
          ', ',
        )}${userContext.masteredWords.length > 5 ? ` and ${userContext.masteredWords.length - 5} more` : ''}
- Recent Practice: ${userContext.recentWords
        .slice(0, 5)
        .map((w) => `"${w.word}" (${w.language}, ${w.correct ? '✓' : '✗'})`)
        .join(', ')}
      ...
      tools: {
        translateWord: tool({
          ...
          inputSchema: z.object({
            ...
            targetLanguages: z
              .array(z.string())
              .describe('Target language codes, or all if not specified')
              .optional(),
          }),
        ...
        getWordSuggestions: tool({
          description:
            'Get word suggestions for learning based on a language or topic',
          inputSchema: z.object({
            languageCode: z
              .string()
              .describe('Language code to get words from')
              .optional(),
          ...
          execute: async ({ languageCode, count = 5 }) => {
            try {
              ...
              const wordRows = await db
                ...
                .leftJoin(languagesT, eq(wordsT.languageId, languagesT.id))
                .where(
                  languageId ? eq(wordsT.languageId, languageId) : undefined,
                )
                .limit(count);
        ...
        getUserStats: tool({
          ...
          inputSchema: z.object({
            languageCode: z
              .string()
              .describe('Specific language to get stats for')
              .optional(),
          }),
          // @ts-ignore - execute function is valid in Vercel AI SDK
            ...
            try {
              ...
              return {
                ...
                languages: (stats.languages || []).map((lang: any) => ({
                  ...
                  progress:
                    lang.totalWords > 0
                      ? Math.round((lang.mastered / lang.totalWords) * 100)
                      : 0,
        ...
        getUserLikedWords: tool({
          ...
          inputSchema: z.object({
            languageCode: z
              .string()
              .describe('Filter by specific language code')
              .optional(),
            limit: z
              .number()
          ...
          execute: async ({ languageCode }) => {
            try {
              ...
              const likedWords: any[] | null = await safe(
                async () => null as any[] | null,
              );

              const filteredWords = languageCode
                ? likedWords?.filter(
                    (l) => (l.words as any)?.languages?.code === languageCode,
                  ) || []
                : likedWords || [];
        ...
        analyzeImage: tool({
          description:
            "Analyze an image to detect objects and translate them to Aboriginal languages. Automatically uses the user's learning languages unless a specific language is requested. This tool is automatically triggered when a user sends an image.",
          inputSchema: z.object({
            description: z
              .string()
              .describe(
                'The description listing ALL specific objects visible in the image',
              ),
            languages: z
              .array(z.string())
              .optional()
              .describe(
                "Specific language codes to translate to (defaults to user's learning languages)",
              ),
            includeContext: z
              .boolean()
              .default(true)
              .describe('Include cultural context'),
            userRequestedLanguage: z
              .string()
              .optional()
              .describe(
                'Language name mentioned by the user (e.g., "Yalanji") - overrides default languages',
              ),
          }),
          // @ts-ignore - execute function is valid in Vercel AI SDK
          execute: async ({
            ...
            try {
              ...
              const seePattern =
                /(?:see|contains?|includes?|shows?|displays?|features?)\s+(?:a |an |the )?([^,.]+(?:,\s*[^,.]+)*)/gi;
              const matches = Array.from(
                description.matchAll(seePattern),
              ) as RegExpMatchArray[];
              ...
              if (
                ...
                userContext.languages.length > 0
              ) {
              ...
              const translationPromises = detectedItems.map(async (item) => {
                ...
                if (targetLanguageIds.length > 0) {
                  conditions.push(
                    inArray(wordsT.languageId, targetLanguageIds),
                  );
                }
                ...
                const exactDefs = await primaryDefinitions(
                  ...
                  try {
                    ...
                    if (similarWords && similarWords.length > 0) {
                      ...
                      translations = similarWords.map((w: any) => ({
                        word: w.word,
                        languages: {
                          name: w.language_name,
                          code: w.language_code,
                        },
                  ...
                  translations:
                    translations?.map((t) => {
                      ...
                      return {
                        ...
                        culturalContext: includeContext
                          ? `The word "${t.word}" in ${tl?.name} ${searchMethod === 'similarity' ? 'is semantically related to' : 'reflects the cultural importance of'} ${item} in Aboriginal communities.`
                          : undefined,
                      };
                    }) || [],
                };
              ...
              if (userRequestedLanguage) {
                ...
                languageNote =
                  'Showing translations from all available Aboriginal languages';
              }

              // Check if any objects used similarity search
              const usedSimilaritySearch = detectedObjects.some(
                (obj) => obj.searchMethod === 'similarity',
              );
              ...
              const result = {
                ...
                detectedObjects: detectedObjects.filter(
                  (obj) => obj.translations.length > 0,
                ),
                ...
                learningTips: [
                  ...
                  ...(usedSimilaritySearch
                    ? [
                        'Some words shown are semantically related concepts when exact translations were not found',
                      ]
                    : []),
                ],
                relatedWords:
                  relatedWords.length > 0 ? relatedWords : undefined,
              };
