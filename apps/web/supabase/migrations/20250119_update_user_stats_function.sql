-- Update the user stats function to support language filtering
CREATE OR REPLACE FUNCTION get_user_stats(p_user_id uuid, p_language_code text DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
    ...
    v_language_id uuid;
BEGIN
    -- Get language ID if code provided
    ...
    WITH attempt_stats AS (
        ...
        AND (v_language_id IS NULL OR EXISTS (
            SELECT 1 FROM words w 
            WHERE w.id = qa.word_id 
            AND w.language_id = v_language_id
        ))
    ),
    recent_7_days AS (
        SELECT 
    ...
    word_stats AS (
        SELECT 
            ...
            COUNT(DISTINCT srs.word_id) FILTER (WHERE srs.bucket = 5) AS mastered_words,
            COUNT(DISTINCT srs.word_id) FILTER (WHERE srs.due_date <= NOW()) AS due_words
        FROM spaced_repetition_states srs
        JOIN words w ON w.id = srs.word_id
        WHERE srs.user_id = p_user_id
        AND (v_language_id IS NULL OR w.language_id = v_language_id)
    ),
    language_stats AS (
        SELECT 
            l.code,
        ...
        WHERE (v_language_id IS NULL OR l.id = v_language_id)
        AND EXISTS (
    ...
    streak_calc AS (
        ...
        FROM (
            ...
            FROM (
                SELECT 
                    DATE(qa.created_at) AS attempt_date,
                    DATE(qa.created_at) - ROW_NUMBER() OVER (ORDER BY DATE(qa.created_at)) * INTERVAL '1 day' AS streak_group
                FROM quiz_attempts qa
                WHERE qa.user_id = p_user_id
                AND (v_language_id IS NULL OR EXISTS (
                    SELECT 1 FROM words w 
